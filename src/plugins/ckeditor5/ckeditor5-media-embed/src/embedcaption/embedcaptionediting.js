...
/**
 * @module image/imagecaption/Embedcaptionediting
 */
 ...
 */
export default class EmbedCaptionEditing extends Plugin {
  /**
   * @inheritDoc
   */
  ...
  init() {
    ...
    editor.model.document.registerPostFixer(writer =>
      this._insertMissingModelCaptionElement(writer)
    );
  ...
  _insertMissingModelCaptionElement(writer) {
    ...
    const mediasWithoutCaption = [];

    for (const entry of changes) {
			// console.log("lckhanhs => entry:", entry);
			
      if (entry.type == "insert" && entry.name != "$text") {
        const item = entry.position.nodeAfter;
				// console.log("lckhanhs => item: ", item);
				
        if (item.name == "media" && !getCaptionFromImage(item)) {
          console.log("nghiepto:",item);
          
          mediasWithoutCaption.push(item);
        }
        ...
        //       mediasWithoutCaption.push(nestedItem);
        //     }
        //   }
        // }
      }
    ...
    for (const image of mediasWithoutCaption) {
      writer.appendElement("embedCaption", image);
    }

    return !!mediasWithoutCaption.length;
  }
}
...
function captionModelToView(elementCreator, hide = true) {
  // console.log("lckhanhs => elementCreator: ", elementCreator);

  return (evt, data, conversionApi) => {
    const captionElement = data.item;
		// console.log("lckhanhs => captionElement: ", captionElement);
...
function insertViewCaptionAndBind(
  ...
  conversionApi
	...
	console.log("lckhanhs => viewCaption: ", viewCaption);
	console.log("lckhanhs => modelCaption: ", modelCaption);
	console.log("lckhanhs => viewImage: ", viewImage);
	console.log("lckhanhs => conversionApi: ", conversionApi);
...
function getParentCaption(node) {
  ...
  const caption = ancestors.find(ancestor => ancestor.name == "embedCaption");
