...
class PhotoList extends Component{
  ...
  constructor() {
    ...
    this.state = {
      ...
      showModal: false,
      endpoint: 'royalties-photographers',
      type: 'Nhiếp ảnh'
    };
  }
  componentDidMount(){
    ...
    this.props.getRoyaltiesPhoto(filters);
    // this.getData();    
   }
   ...
   UNSAFE_componentWillReceiveProps(nextProps){
      ...
      const { config: { listRoyaltiesPhoto : nlistRoyaltiesPhoto } } = nextProps;
      const { config: { listRoyaltiesPhoto } } = this.props;
      if(nlistRoyaltiesPhoto != listRoyaltiesPhoto){
        this.getData(nextProps);    
      } 
   }
 
   getData(props){
     const { config: { listRoyaltiesPhoto, countData } } = props || this.props;
     this.setState({listRoyaltiesPhoto: listRoyaltiesPhoto, totalItemCount: countData })  
   }
      ...
      () => {
        ...
        if (execute) {
          ...
          if (queryString != "") {
            ...
            this.props.history.push(`/content/photo?${queryString}`);
          } else {
            this.props.history.push(`/content/photo`);
          }
          this.props.getRoyaltiesPhoto(this.state.filters);
        }
  ...
  resetFilters(filter) {
    ...
    this.setState({listRoyaltiesPhoto: [],addedContent: null });
    return filters;
  }
  ...
  render() {
    ...
    const { listRoyaltiesPhoto } = this.state
      return (
                      ...
                      <AddItem endpoint={this.state.endpoint} type={this.state.type} getdata={this.searchDataByLabel} datasearch={this.state.filters.label.value} addeddata={this.addedData} />
                        ...
                        <div className="card-body table-responsive p-0">
                          ...
                          <table className="table table-hover">
                            ...
                            <tbody>
                            {this.state.addedContent && <ContentItem item={this.state.addedContent} key={`user-${this.state.addedContent.id}`} type={this.state.type} endpoint={this.state.endpoint} /> }
                            {
                            listRoyaltiesPhoto && listRoyaltiesPhoto.map((item, idx) => {
                              return (
                                <ContentItem item={item} key={`user-${idx}`} type={this.state.type} endpoint={this.state.endpoint} />                     
                              )
                            })
                            }
                               {(listRoyaltiesPhoto && listRoyaltiesPhoto.length == 0) && <tr className="odd">
                              <td
                                valign="top"
                                colSpan="8"
